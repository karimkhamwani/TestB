...
test('book freshness: a quiet book on a live connection stays fresh', () => {
  const t0 = 1_000_000;
  const b = new Book('X');
  b.applySnapshot({ bids: [{ price: 0.5, size: 10 }], asks: [{ price: 0.52, size: 10 }] }, t0);
  // No changes for 90s, but the connection kept delivering messages.
  const clock = { t: t0 + 90_000 };
  b.feedClock = clock;
  assert.equal(b.top().tsMs, t0 + 90_000, 'freshness follows the feed clock');
  // Dead connection: clock stops, the book goes stale with it.
  clock.t = t0 + 90_000; // frozen
  const ageAt = (now) => now - b.freshTs();
  assert.ok(ageAt(t0 + 95_000) === 5_000);
  // Without a clock (rest-poll books), freshness falls back to last change.
  const c = new Book('Y');
  c.applySnapshot({ bids: [], asks: [] }, t0);
  assert.equal(c.freshTs(), t0);
});
