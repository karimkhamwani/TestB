...
// Observer: measures whether the opportunities exist
...
//   data/arb-journal.json   — pair attempts (written by the ledger, not here)
...
class Observer {
  ...
  _closeEvent(key, ev, nowMs) {
    ...
    if (gate || this.cfg.verbose) {
      this.logFn(`EDGE ${ev.side} ${ev.series} ${durationMs}ms maxEdge=${ev.maxEdge} minFeeAdjSum=${ev.minFeeAdjSum}` +
        `${gate ? ' [GATE]' : ''}${ev.execOk ? ' executable' : ''}`);
    }
    this._append({
  ...
  recordNearResOutcome(ring, outcome) {
    ...
    if (outcome !== null && last.leading !== outcome) {
      this.logFn(`near-res REVERSAL ${ring.series} ${ring.window}: led ${last.leading} in final 30s, resolved ${outcome}`);
    }
    this._append({
  ...
  writeStatus(books, feedState, nowMs = Date.now(), extra = {}) {
    ...
    for (const [series, mkt] of this.markets) {
      const up = books.get(mkt.upToken);
      const down = books.get(mkt.downToken);
      const fresh = (b) => (b && b.hasSnapshot ? nowMs - (b.freshTs ? b.freshTs() : b.tsMs) : null);
      const uTop = up && up.hasSnapshot ? up.top() : null;
      const dTop = down && down.hasSnapshot ? down.top() : null;
      perSeries[series] = {
        window: mkt.slug,
        windowEndsInSec: Math.round((mkt.windowEndMs - nowMs) / 1000),
        bookAgeMsUp: fresh(up),
        bookAgeMsDown: fresh(down),
        askUp: uTop ? uTop.ask : null,
        askDown: dTop ? dTop.ask : null,
        bidUp: uTop ? uTop.bid : null,
        bidDown: dTop ? dTop.bid : null,
        askSum: uTop && dTop && uTop.ask != null && dTop.ask != null ? core.round6(uTop.ask + dTop.ask) : null,
        bidSum: uTop && dTop && uTop.bid != null && dTop.bid != null ? core.round6(uTop.bid + dTop.bid) : null,
