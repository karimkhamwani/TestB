...
class Strategy {
  ...
  /** A public trade printed. In dry mode this is the ONLY thing that fills a
   *  resting maker bid — the book alone never crosses our own quote. */
  async onTrade(tokenId, trade, books, nowMs = Date.now()) {
    if (this.halted || !this.maker || !this.venue.onTrade) return;
    this.books = books;
    for (const fill of this.venue.onTrade(tokenId, trade)) {
      await this.maker.onFill(fill, books, nowMs);
    }
  }
