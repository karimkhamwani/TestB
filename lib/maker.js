...
class Maker {
  ...
  desiredQuotes(input, mkt) {
    ...
    // Respect the detector's price band: quoting a 0.97 favorite is
    // near-resolution tail risk, not market making.
    const inBand = (q) => q !== null && q >= Math.max(tick, this.cfg.detector.priceBandMin) && q <= this.cfg.detector.priceBandMax;
