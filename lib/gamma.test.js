...
test('config: .env parser strips inline comments but not quoted hashes', () => {
  const fs = require('node:fs');
  const os = require('node:os');
  const path = require('node:path');
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'arb-env-'));
  fs.writeFileSync(path.join(dir, '.env'),
    'ARB_BUY_SUM=0.985                # fee-adjusted trigger, buy side\n' +
    '# full-line comment\n' +
    'ARB_SERIES=eth-updown-5m\n' +
    'POLY_API_PASSPHRASE="secret#hash"  \n');
  const oldEnv = { ...process.env };
  const oldCwd = process.cwd();
  try {
    for (const k of ['ARB_BUY_SUM', 'ARB_SERIES', 'POLY_API_PASSPHRASE', 'UPDOWN_SLUG_PREFIX']) delete process.env[k];
    process.chdir(dir);
    const cfg = parseConfig({ argv: [] });
    assert.equal(cfg.detector.buySum, 0.985);
    assert.deepEqual(cfg.series, ['eth-updown-5m']);
    assert.equal(process.env.POLY_API_PASSPHRASE, 'secret#hash');
  } finally {
