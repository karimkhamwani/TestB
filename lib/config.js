...
function parseConfig({ argv = process.argv } = {}) {
  ...
  const mode = argv.includes('--observe') ? 'observe' : (process.env.ARB_MODE || 'observe');
  ...
  return {
    ...
    observer: {
      minEventMs: num('ARB_OBS_MIN_EVENT_MS', 300),  // Phase 0 gate: events >= 300ms
    ...
    // Phase 0 gate; it exists only to smoke-test plumbing.
