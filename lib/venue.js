'use strict';

// Order venues behind one interface, so the executor/trader/ledger code is
// identical in dry-run and live:
//
//   venue.buyFAK({tokenId, price, size, feeRateBps, tickSize}) ->
//     { ok, filledShares, usdc, avgPrice, ackMs, orderId, status, error, paper }
//   venue.sellFAK(...) same shape
//
// FAK = fill-and-kill: whatever is marketable fills immediately, the rest is
// cancelled — never rests on the book. We book ONLY the exchange-confirmed
// makingAmount/takingAmount (mm-bot fix #3), never the requested size.

const core = require('../arb-core');

/** Paper venue: simulates FAK fills against the live local books.
 *  Optimistic by construction — it assumes the displayed size is real and
 *  that we win the race. Dry-run P&L is therefore an UPPER BOUND.
 *
 *  Resting (GTC) orders are simulated too: a paper BUY resting at price P
 *  fills when the market's best ask drops to <= P (someone selling into the
 *  bid), a paper SELL when the best bid rises to >= P. checkResting() is
 *  called by the strategy on every book update and returns fill events. */
class PaperVenue {
  constructor(books) {
    this.books = books; // Map assetId -> Book (lib/books.js)
    this.kind = 'paper';
    this.resting = new Map(); // orderId -> {tokenId, side, price, size, remaining}
    this.nextRestId = 1;
  }

  async postGTC({ tokenId, side, price, size }) {
    const orderId = `paper-gtc-${this.nextRestId++}`;
    // Queue position: everything already resting at our price level trades
    // ahead of us. Without this the sim fills us on the first print at our
    // level and wildly overstates maker fill rates.
    const book = this.books.get(tokenId);
    const queueAhead = book ? book.sizeAt(side === 'BUY' ? 'BUY' : 'SELL', price) : 0;
    this.resting.set(orderId, { orderId, tokenId, side, price, size, remaining: size, queueAhead });
    return { ok: true, orderId, status: 'live', error: null, paper: true };
  }

  /** Simulate resting fills against REAL taker flow (last_trade_price prints).
   *  A resting BUY fills when a taker SELLS at or below our price; the print's
   *  volume first consumes the queue ahead of us, then fills us. A print
   *  strictly through our level means our level was swept — queue is gone. */
  onTrade(tokenId, { price, size, side }) {
    const fills = [];
    const takerSold = String(side).toUpperCase() === 'SELL';
    for (const o of [...this.resting.values()]) {
      if (o.tokenId !== tokenId) continue;
      const hit = o.side === 'BUY'
        ? (takerSold && price <= o.price + 1e-9)
        : (!takerSold && price >= o.price - 1e-9);
      if (!hit) continue;
      const through = o.side === 'BUY' ? price < o.price - 1e-9 : price > o.price + 1e-9;
      if (through) o.queueAhead = 0;
      let avail = size;
      const eaten = Math.min(avail, o.queueAhead);
      o.queueAhead -= eaten;
      avail -= eaten;
      const filled = Math.min(o.remaining, avail);
      if (filled <= 1e-9) continue;
      o.remaining -= filled;
      if (o.remaining <= 1e-9) this.resting.delete(o.orderId);
      // Maker fills are FEE-FREE under takerOnly — no fee term here on purpose.
      fills.push({ orderId: o.orderId, tokenId, side: o.side, price: o.price, shares: filled, usdc: filled * o.price, paper: true });
    }
    return fills;
  }

  async cancel(orderId) {
    const had = this.resting.delete(orderId);
    return { ok: had, orderId, paper: true };
  }

  /** True when we have a resting order at this token's best level (self-trade guard). */
  restingAtBest(tokenId, side) {
    const book = this.books.get(tokenId);
    if (!book) return false;
    const best = side === 'BUY' ? book.bestBid() : book.bestAsk();
    for (const o of this.resting.values()) {
      if (o.tokenId === tokenId && o.side === side && best !== null && o.price === best) return true;
    }
    return false;
  }

  /** Simulate resting-order fills against the current books; returns fills. */
  checkResting(tokenId) {
    const fills = [];
    const book = this.books.get(tokenId);
    if (!book) return fills;
    for (const o of [...this.resting.values()]) {
      if (o.tokenId !== tokenId) continue;
      let filled = 0;
      if (o.side === 'BUY') {
        const ask = book.bestAsk();
        if (ask !== null && ask <= o.price) filled = Math.min(o.remaining, book.sizeAt('SELL', ask));
      } else {
        const bid = book.bestBid();
        if (bid !== null && bid >= o.price) filled = Math.min(o.remaining, book.sizeAt('BUY', bid));
      }
      if (filled > 0) {
        o.remaining -= filled;
        if (o.remaining <= 1e-9) this.resting.delete(o.orderId);
        fills.push({ orderId: o.orderId, tokenId, side: o.side, price: o.price, shares: filled, usdc: filled * o.price, paper: true });
      }
    }
    return fills;
  }

  // FAK fills are TAKER fills and pay the V2 fee curve — dry P&L must carry
  // that cost or every completion/unwind looks ~2c/share better than reality.
  // (Resting GTC fills stay fee-free: makers are exempt under takerOnly.)
  async buyFAK({ tokenId, price, size, feeSchedule }) {
    const book = this.books.get(tokenId);
    const ask = book ? book.bestAsk() : null;
    if (ask == null || ask > price) {
      return { ok: true, filledShares: 0, usdc: 0, avgPrice: null, ackMs: 0, orderId: null, status: 'unmatched', error: null, paper: true };
    }
    const filled = Math.min(size, book.sizeAt('SELL', ask));
    const fee = filled * core.takerFeePerShare(ask, feeSchedule);
    const usdc = filled * ask + fee; // buyer pays price plus fee
    return { ok: true, filledShares: filled, usdc, avgPrice: filled ? usdc / filled : null, feeUsdc: fee, ackMs: 0, orderId: `paper-${Date.now()}`, status: 'matched', error: null, paper: true };
  }

  async sellFAK({ tokenId, price, size, feeSchedule }) {
    const book = this.books.get(tokenId);
    const bid = book ? book.bestBid() : null;
    if (bid == null || bid < price) {
      return { ok: true, filledShares: 0, usdc: 0, avgPrice: null, ackMs: 0, orderId: null, status: 'unmatched', error: null, paper: true };
    }
    const filled = Math.min(size, book.sizeAt('BUY', bid));
    const fee = filled * core.takerFeePerShare(bid, feeSchedule);
    const usdc = filled * bid - fee; // seller receives price minus fee
    return { ok: true, filledShares: filled, usdc, avgPrice: filled ? usdc / filled : null, feeUsdc: fee, ackMs: 0, orderId: `paper-${Date.now()}`, status: 'matched', error: null, paper: true };
  }
}

/** Pull a USABLE reason out of a CLOB response or a thrown client error.
 *  The API puts its real complaint in the HTTP body; axios hides that in
 *  err.response.data, and a non-2xx response carries it in .data/.error while
 *  errorMsg stays empty. Reading only errorMsg/err.message yields a bare
 *  "400", which is what made this failure opaque for hours. */
function describeOrderError(resp, err) {
  const body = (d) => (d == null ? null : typeof d === 'string' ? d : JSON.stringify(d));
  if (err) {
    const b = body(err.response?.data);
    return b ? `${err.message} :: ${b}` : err.message;
  }
  const b = body(resp?.data ?? resp?.error ?? resp?.detail);
  return resp?.errorMsg || resp?.message || b
    || (resp?.status ? `HTTP ${resp.status} (no body)` : 'unknown');
}

/** Live venue: real orders on the Polymarket CLOB via the official client.
 *  Requires POLY_PRIVATE_KEY (+ optionally POLY_FUNDER_ADDRESS for the proxy
 *  wallet, POLY_SIGNATURE_TYPE, and POLY_API_KEY/SECRET/PASSPHRASE — derived
 *  from the key when absent). Loaded lazily: observe/dry never touch this. */
class LiveVenue {
  constructor(cfg) {
    this.cfg = cfg;
    this.kind = 'live';
    this.client = null;
    this.Side = null;
    this.OrderType = null;
  }

  async init() {
    const pk = process.env.POLY_PRIVATE_KEY;
    if (!pk) throw new Error('live mode requires POLY_PRIVATE_KEY in the environment/.env');
    // CLOB API **v2**. The v1 package (@polymarket/clob-client) is archived and
    // every order it signs is rejected with "invalid order version" — its
    // CLOB_VERSION is hardcoded to "1". v2 ships a CJS build, so unlike v1 this
    // needs no dynamic import.
    const mod = require('@polymarket/clob-client-v2');
    const { Wallet } = require('ethers');
    const signer = new Wallet(pk);
    const funder = process.env.POLY_FUNDER_ADDRESS || signer.address;
    const sigType = Number(process.env.POLY_SIGNATURE_TYPE ?? 1); // 1 = POLY_PROXY
    // v2 takes ONE options object; v1 took positional args.
    const base = {
      host: this.cfg.clobBase,
      chain: 137,
      signer,
      signatureType: sigType,
      funderAddress: funder,
    };
    const creds = process.env.POLY_API_KEY
      ? { key: process.env.POLY_API_KEY, secret: process.env.POLY_API_SECRET, passphrase: process.env.POLY_API_PASSPHRASE }
      : await new mod.ClobClient(base).createOrDeriveApiKey();
    this.client = new mod.ClobClient({ ...base, creds });
    this.creds = creds; // kept for the user-channel websocket (our fills, pushed)
    this.Side = mod.Side;
    this.OrderType = mod.OrderType;
    this.address = funder;
    this.signerAddress = signer.address;
  }

  async _fak(side, { tokenId, price, size, feeRateBps, tickSize }) {
    const t0 = Date.now();
    try {
      // tickSize/negRisk passed explicitly so createOrder does no REST lookups
      // on the hot path (updown markets: tick 0.01, negRisk false — from discovery).
      // v2 orders carry NO feeRateBps — fees come from the exchange's schedule
      // rather than being signed into the order. tickSize/negRisk stay explicit
      // so createOrder does no REST lookups on the hot path (updown: 0.01/false).
      const order = await this.client.createOrder(
        { tokenID: tokenId, price, side: side === 'BUY' ? this.Side.BUY : this.Side.SELL, size },
        { tickSize: String(tickSize ?? 0.01), negRisk: false },
      );
      const resp = await this.client.postOrder(order, this.OrderType.FAK);
      const ackMs = Date.now() - t0;
      const making = Number(resp?.makingAmount || 0);
      const taking = Number(resp?.takingAmount || 0);
      const filledShares = side === 'BUY' ? taking : making;
      const usdc = side === 'BUY' ? making : taking;
      return {
        ok: !!resp?.success,
        filledShares,
        usdc,
        avgPrice: filledShares > 0 ? usdc / filledShares : null,
        ackMs,
        orderId: resp?.orderID || null,
        status: resp?.status || null,
        error: resp?.success ? null : describeOrderError(resp),
        raw: resp?.success ? undefined : resp,
        paper: false,
      };
    } catch (err) {
      return { ok: false, filledShares: 0, usdc: 0, avgPrice: null, ackMs: Date.now() - t0, orderId: null, status: 'error', error: describeOrderError(null, err), paper: false };
    }
  }

  buyFAK(args) { return this._fak('BUY', args); }
  sellFAK(args) { return this._fak('SELL', args); }

  /** Rest a maker order on the book. Maker fills pay ZERO fees (takerOnly). */
  async postGTC({ tokenId, side, price, size, feeRateBps, tickSize }) {
    try {
      const order = await this.client.createOrder(
        { tokenID: tokenId, price, side: side === 'BUY' ? this.Side.BUY : this.Side.SELL, size },
        { tickSize: String(tickSize ?? 0.01), negRisk: false },
      );
      // v2 postOrder(order, orderType, postOnly, deferExec) — note the arg order
      // differs from v1's (order, orderType, deferExec, postOnly).
      const resp = await this.client.postOrder(order, this.OrderType.GTC);
      return {
        ok: !!resp?.success,
        orderId: resp?.orderID || null,
        status: resp?.status || null,
        error: resp?.success ? null : describeOrderError(resp),
        raw: resp?.success ? undefined : resp,
        paper: false,
      };
    } catch (err) {
      return { ok: false, orderId: null, status: 'error', error: describeOrderError(null, err), paper: false };
    }
  }

  async cancel(orderId) {
    try {
      const resp = await this.client.cancelOrder({ orderID: orderId });
      return { ok: !resp?.not_canceled?.length, orderId, raw: resp, paper: false };
    } catch (err) {
      return { ok: false, orderId, error: err.message, paper: false };
    }
  }

  restingAtBest() { return false; } // live self-trade handling is exchange-side
  checkResting() { return []; }     // live fills arrive on the user websocket instead
  onTrade() { return []; }          // ditto — public prints are not OUR fills
}

module.exports = { PaperVenue, LiveVenue };
